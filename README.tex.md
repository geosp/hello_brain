...
3. For each item in the dataset `forward propagate` by invoking the activation function on perceptron $P$ from $L_{1h...nh}$ (all hidden layers) to $L_{o}$ (the output layer) using the input value for each item in the data set as the input. The signals of perceptrons in a previous layer became the input for $\hat W_{n}$ for the current layer.
